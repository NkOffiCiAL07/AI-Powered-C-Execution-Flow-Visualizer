...
function ShareBar({ article }) {
  ...
  const pageUrl = `${window.location.origin}${window.location.pathname}?v=blog`;
  const tweetText = `${article.title} — via @traceon_dev\n\n${article.excerpt.slice(0, 120)}…`;
  ...
  return (
    <div className="blog-share-bar">
      ...
      <div className="blog-share-btns">
        <a
          className="blog-share-btn blog-share-btn--twitter"
        ...
        <button
          className={`blog-share-btn blog-share-btn--copy ${copied ? 'copied' : ''}`}
          onClick={handleCopy}
          title="Copy link"
        >
